...
aiEvents* aiEvents::generateEvent()
{
	if (personA->dailyEvents[eventTime] == NULL)
	{
		...
		{			
			...
			{
				Eating eating = Eating((*it),aiList, eventTime);
				(*it)->dailyEvents[eventTime] = eating.generateEating();
			}
		}
		else if (eventType == WORKING)
		{
			for (std::vector<aiPerson*>::iterator it = aiInvolved.begin(); it != aiInvolved.end(); it++)
			{
				Working working = Working((*it),aiList, eventTime);
				(*it)->dailyEvents[eventTime] = working.generateWorking();
			}
		}
		else if (eventType == RESTING)
		{
			for (std::vector<aiPerson*>::iterator it = aiInvolved.begin(); it != aiInvolved.end(); it++)
			{
				Resting resting = Resting((*it),aiList, eventTime);
				(*it)->dailyEvents[eventTime] = resting.generateResting();
			}
		}
		...
		{	
			for (std::vector<aiPerson*>::iterator it = aiInvolved.begin(); it != aiInvolved.end(); it++)
			{
				Fighting fighting = Fighting((*it),aiList, eventTime);
				(*it)->dailyEvents[eventTime] = fighting.generateFighting();
			}
		}
		else if (eventType == OTHER)
		{
			for (std::vector<aiPerson*>::iterator it = aiInvolved.begin(); it != aiInvolved.end(); it++)
			{	
				Other other = Other((*it),aiList, eventTime);
				(*it)->dailyEvents[eventTime] = other.generateOther();
			}
		}
	}
}
...
Working* Working::generateWorking()
{
...
Working* Working::generateWorkingInterruptive()
{
	for (std::vector<Relation*>::iterator it = personA->relationList.begin(); it != personA->relationList.end(); it++)
	{
		if ((*it)->relations == COLLEGUE)
...
Eating* Eating::generateEatingInterruptive()
{
	for (std::vector<Relation*>::iterator it = personA->relationList.begin(); it != personA->relationList.end(); it++)
	{
...
Fighting* Fighting::generateFighting()
{
	switch (eventTime)
	{
...
Fighting* Fighting::generateFightingInterruptive()
{
...
Resting* Resting::generateResting()
{
	switch (eventTime)
	{
...
Resting* Resting::generateRestingInterruptive()
{
...
Other* Other::generateOther()
{
}

Other* Other::generateOtherInterruptive()
{
}
...
bool aiEngine::simulate()
{
	...
	while (true)
	{
		...
		for (std::vector<aiPerson*>::iterator it = aiList.begin(); it != aiList.end(); it++)
		{
			for (int i = 0; i < 8; i++)
			{
				aiEvents((*it), aiList, (ai_time)i).generateEffect;
				(*it)->updateBuff(i);
